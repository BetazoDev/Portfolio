...
app.get('/api/projects', async (_req, res) => res.json(await prisma.project.findMany({ where: { status: ProjectStatus.published, showOnHomepage: true }, orderBy: [{ featured: 'desc' }, { sortOrder: 'asc' }], include: projectInclude })));
app.get('/api/projects/home', async (_req, res) => res.json(await prisma.project.findMany({ where: { status: ProjectStatus.published, showOnHomepage: true }, orderBy: [{ featured: 'desc' }, { sortOrder: 'asc' }], include: projectInclude })));
