...
app.get('/api/projects', async (_req, res) => res.json(await prisma.project.findMany({ where: { status: ProjectStatus.published }, orderBy: [{ featured: 'desc' }, { sortOrder: 'asc' }], include: projectInclude })));
app.get('/api/projects/:slug', async (req, res) => { const project = await prisma.project.findFirst({ where: { slug: req.params.slug, status: ProjectStatus.published }, include: projectInclude }); if (!project) return res.status(404).json({ error: 'Project not found' }); res.json(project); });
...
app.patch('/api/admin/media/:id', requireAuth, async (req, res) => { const data = z.object({ altText: z.string().nullable().optional(), caption: z.string().nullable().optional() }).parse(req.body); res.json(await prisma.media.update({ where: { id: String(req.params.id) }, data })); });
