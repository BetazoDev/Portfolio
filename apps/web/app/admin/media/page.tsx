...
export default function MediaPage() {
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage(
          detail.error ?? "No se pudo subir. Verifica Supabase Storage.",
        );
        return;
      }
      formElement.reset();
  ...
  async function remove(id: string) {
    if (!confirm("¿Eliminar el archivo del storage y de la biblioteca?"))
      return;
    ...
    <>
      <header className="border-b border-white/10 pb-10">
        ...
        <p className="mt-4 max-w-xl text-sm text-white/45">
          Imágenes para covers, thumbnails, arquitectura y galerías.
        </p>
      </header>
      <form
        onSubmit={upload}
        ...
        <label className="flex min-h-28 cursor-pointer items-center gap-5 border border-white/10 px-6 transition hover:border-[#8b78ff]">
          ...
          <span>
            ...
            <span className="mt-1 block font-mono text-[9px] uppercase tracking-widest text-white/35">
              JPEG, PNG, WEBP o AVIF · máximo 10 MB
            </span>
          </span>
          <input
            name="file"
          ...
          <article key={item.id} className="border border-white/10 bg-[#101011] p-5">
            ...
            <div className="mt-5 flex items-start justify-between gap-4">
              ...
              <button
                ...
                aria-label={`Eliminar ${item.originalFilename}`}
