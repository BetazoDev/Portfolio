...
import { ArrowUpRight, Eye, EyeOff, Search, Trash2 } from 'lucide-react';
    ...
    <>
      <header className="flex flex-wrap items-end justify-between gap-6 border-b border-white/10 pb-10">
        <div>
          ...
          <h1 className="mt-5 text-5xl font-bold md:text-7xl">Projects</h1>
          <p className="mt-4 text-sm text-white/45">
            Manage content with the same visual hierarchy as the portfolio.
          </p>
        </div>
        <Link
      ...
      <label className="my-10 flex items-center gap-4 border-b border-white/15 py-4 focus-within:border-[#a855f7]">
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {shown.map((item, index) => (
          <article
            key={item.id}
            className="group relative min-h-72 overflow-hidden border border-white/10 bg-[#101011] p-6 transition hover:border-[#a855f7]/70"
          >
            ...
            <div className="relative flex h-full min-h-60 flex-col">
              <div className="flex justify-between font-mono text-[9px] uppercase tracking-widest text-white/35">
              ...
              <div className="mt-auto">
                ...
                <p className="mt-3 line-clamp-2 text-xs leading-5 text-white/45">
                  {item.shortSummary || 'No editorial summary.'}
                </p>
              </div>
              <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-5">
                ...
                <div className="flex items-center gap-4">
                  <button
                    ...
                    aria-label={
                      item.showOnHomepage
                        ? `Hide ${item.title} from portfolio`
                        : `Show ${item.title} on portfolio`
                    }
                    title={
                      item.showOnHomepage
